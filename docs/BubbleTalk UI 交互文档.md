...
- 头像（可生成：一致风格的半写实/插画/3D 头像，V1 可先用静态）
- 名称与身份标签（极淡）
- “正在说话”状态指示（动效）
...
- 同一泡泡内头像风格统一
- 头像不需要写实，更像“未来学院的讲者”
- 背景可加角色专属光环色（Economist 冷白/Host 薄荷/用户琥珀）

### 3.3 谁在说话：Speaking Indicator（你要的“显示正在说”）

当某个角色输出时：

- 头像出现“呼吸光环”（不闪烁，像声波缓慢扩散）
- 消息气泡先出现“打字/思考”态（3～6 个微点，不要老套跳点，可用环形粒子）
- 文本逐行出现（可选），但要避免像打字机太慢

语音未来扩展：

- 说话时头像下方出现微弱的音量条（轻，不抢戏）
