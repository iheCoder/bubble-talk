...
Engine 是导演状态机的主控模块，负责协调各子模块的调用顺序与决策逻辑。它相当于“导演大脑”，在每一轮对话中根据当前状态、用户输入和节奏信号，生成一份完整的导演计划（DirectorPlan）。其主要流程如下：

1. **接收输入**：从会话状态中读取用户输入、当前幕（Act）、节奏信号（Tension、Load、OutputClock）等上下文信息。
2. **前置判断**：检测是否触发强制事件（如用户说“结束”“懂了”、输出超时、分支回收等），若触发则直接生成对应的强制拍点（ExitTicket、TransferSwap 等）。
3. **状态识别**：调用 Heuristics 或 Router LLM，对用户心智状态（MindState）与意图（Intent）进行识别，输出初步判断结果。
4. **生成候选拍点**：调用 Scheduler，根据当前幕、心智状态、意图、张力与认知负荷，生成若干候选 Beat（拍点），每个 Beat 都附带潜在的角色与输出动作建议。
5. **打分与选择最优方案**：对候选 Beat 进行多维打分（truthGain、feelCurve、paceFit、penalty 等），选出最合适的 Beat 作为下一步剧情节点。
6. **角色与输出动作分配**：

    * **角色分配**：不同 Beat 对应不同的叙事角色。例如：

        * *Hook / Reveal* 倾向由 **Host** 或 **Expert** 出场，负责引入与解释；
        * *Twist / LensShift* 通常由 **Challenger** 或 **Peer** 扮演，制造认知冲突；
        * *Check / Feynman / Boundary* 适合 **Coach** 或 **Expert**，引导用户输出；
        * *MiniGame / Cooldown* 可由 **Peer** 或 **Host** 执行，缓和节奏。
    * **输出动作分配**：每个 Beat 还会指定用户必须执行的输出动作（OutputAction），如 Recap、Choice、Example、Boundary、Feynman、Transfer 等。
      Beat 的类型直接影响输出动作的层级：

        * *Reveal* 通常触发 **Recap**（复述）；
        * *Twist* 触发 **Boundary**（边界澄清）；
        * *Feynman* 要求 **Explain**（用自己的话讲解）；
        * *Transfer* 强制 **Apply**（迁移应用）。
          通过这种映射，Engine 能确保每个剧情节点都推动用户输出层级上升。
7. **护栏修正**：调用 Guardrails，对生成的计划进行硬约束校验（如输出超时、分支深度、节奏平衡等），必要时自动修正 Beat、角色或输出动作。
8. **输出计划**：生成最终的 DirectorPlan（包含 NextBeat、NextRole、OutputAction、StackAction 等），交由演员模块（ActorEngine）执行，生成具体台词与交互提示。
